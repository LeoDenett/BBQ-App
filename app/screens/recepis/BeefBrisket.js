...
const BeefBrisket = ({navigation}) => {
  //Firebase Firestore
  const [ loading, setLoading ] = useState(true);
  const [recepi, setRecepi] = useState([]);
  const ref = firebase.firestore().collection('BeefBrisket').get();

  useEffect(() => {
      const showRecepis = firebase.firestore()
          .collection('BeefBrisket')
                ...
                <View>
                    ...
                    <View style={[RecepiStyle.section, {marginVertical: 32} ]}>
                        ...
                        <View>
                            ...
                            />
                            <List.Item
                                title={item.subtitlePreparation8}
                                description={item.preparation8}
                                descriptionNumberOfLines= {20}
                                titleStyle={{color: "#B85C38"}}
                                descriptionStyle={{color: "#E0C097"}}
                            />
                            <List.Item
                                title={item.subtitlePreparation9}
                                description={item.preparation9}
                                descriptionNumberOfLines= {20}
                                titleStyle={{color: "#B85C38"}}
                                descriptionStyle={{color: "#E0C097"}}
                            />
