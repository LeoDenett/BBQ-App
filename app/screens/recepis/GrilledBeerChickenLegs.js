...
const GrilledBeerChickenLegs = ({navigation}) => {
  //Firebase Firestore
  const [ loading, setLoading ] = useState(true);
  const [recepi, setRecepi] = useState([]);
  const ref = firebase.firestore().collection('GrilledBeerChickenLegs').get();

  useEffect(() => {
      const showRecepis = firebase.firestore()
          .collection('GrilledBeerChickenLegs')
...
export default GrilledBeerChickenLegs;
