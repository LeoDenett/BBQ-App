...
const GrilledRibeyeSteack = ({navigation}) => {
  //Firebase Firestore
  const [ loading, setLoading ] = useState(true);
  const [recepi, setRecepi] = useState([]);
  const ref = firebase.firestore().collection('GrilledRibeyeSteack').get();

  useEffect(() => {
      const showRecepis = firebase.firestore()
          .collection('GrilledRibeyeSteack')
...
export default GrilledRibeyeSteack;
